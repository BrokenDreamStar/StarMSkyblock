...
约 30 个 Java 文件中的 `MessageUtil.sendMessage(...)` 调用全部改为 `MessageUtil.send(sender, key)` 或 `MessageUtil.send(sender, key, Map.of(...))`，`MessageUtil.broadcast(...)` 改为 `MessageUtil.broadcast(key)` 或 `MessageUtil.broadcast(key, Map.of(...))`。
...
**After:**
```java
MessageUtil.broadcast("island.create.broadcast", Map.of("player", player.getName()));
```
