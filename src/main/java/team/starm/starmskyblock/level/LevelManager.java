...
public class LevelManager {
    ...
    private void onCalculationComplete(Island island, LevelResults results, Player player) {
        ...
        islandManager.getIslandRepository().updateLevel(
                island.getId(),
                results.getLevel(),
                results.getTotalExperience(),
                serializeBlockCounts(results.getBlockCounts())
        );

        // 发送结果消息
        sendLevelResults(player, island, results, oldLevel);
    }

    /**
     * 发送等级计算结果
     */
    private void sendLevelResults(Player player, Island island, LevelResults results, int oldLevel) {
        int levelChange = results.getLevel() - oldLevel;

        MessageUtil.sendMessage(player, "");
        MessageUtil.sendMessage(player, "&a=== 岛屿等级计算完成 ===");
        MessageUtil.sendMessage(player, "&a等级: &f" + oldLevel + " &7→ &a" + results.getLevel()
        ...
        if (!results.getBlocksOverLimit().isEmpty()) {
            StringBuilder sb = new StringBuilder("&e超出阈值的方块: ");
            for (Map.Entry<Material, Long> entry : results.getBlocksOverLimit().entrySet()) {
                if (entry.getValue() > 0) {
                    sb.append("&f").append(entry.getKey().name()).append("(&c+").append(entry.getValue()).append("&f) ");
                }
            }
